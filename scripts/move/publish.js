...
async function publish() {
  ...
  move
    .createObjectAndPublishPackage({
      ...
      addressName: "trading_bot_addr",
      namedAddresses: {
        // Publish module to new object, but since we create the object on the fly, we fill in the publisher's account address here
        trading_bot_addr: process.env.NEXT_MODULE_PUBLISHER_ACCOUNT_ADDRESS,
      },
      extraArguments: [`--private-key=${process.env.NEXT_MODULE_PUBLISHER_ACCOUNT_PRIVATE_KEY}`,`--url=${aptosSDK.NetworkToNodeAPI[process.env.NEXT_PUBLIC_APP_NETWORK]}`, `--assume-yes`],
    })
    .then((response) => {
      const filePath = ".env";
      let envContent = "";
